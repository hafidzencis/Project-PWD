...
if(isset($_POST['submit']))
	{
		...
		if (addphotoproduct($_POST) == 1) {
            echo "<script>alert('Thank you for send evidence product');
...
<body>  
    ...
    <div class="mb-5 pt-5">
                ...
                <h2 class="text-success"> Evidence Product Has Been Arrived </h2>
                <br>
            
                </div>
                <?php foreach ($pemb as $dat )   {?>
                <h4 class="h4 text-danger d-flex align-item-center justify-content-center 2 pb-3">Please send photo product</h4>
                <?php } ?>       
                ...
                <?php
                    if (!empty($sudahbayar[0]['buktifoto']) and !empty($sudahbayar[0]['tanggalbultifoto'])) {
                        foreach ($sudahbayar as $sb ) {
                            if ($sb['buktifoto'] and $sb['tanggalbultifoto']) { ?>
                           ...
                           <input type="text" class="form-control bg-light border-light mb-4 " value="<?= $sb['tanggalbultifoto'] ?>" required>
    
                                <img src="img/<?=$sb['buktifoto'] ?>" width="500px" alt="">
                       </form>
